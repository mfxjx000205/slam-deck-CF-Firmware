...
#include "math.h"
#include "debug.h"
#define IMAGE_PROCESS_DEBUG_PRINT 1
...
FlyCommand_t Process_ToF_Image(VL53L5CX_ResultsData* p_tof_results)
{
    ...
    uint8_t ToF_Reflectance[ToF_TARGETS_STATUS_LEN];
    ...
    memcpy(ToF_Reflectance, (uint8_t *)(&p_tof_results->reflectance[0]), ToF_TARGETS_STATUS_LEN);
    ...
    //print the distance matrix 8x8
    #ifdef IMAGE_PROCESS_DEBUG_PRINT
    DEBUG_PRINT("--------------------------------\n");
    DEBUG_PRINT("ToFDeck P.I. Distance Matrix:\n");
    for (int i = 0; i < ROW; ++i)
    {
        for (int j = 0; j < COL; ++j)
        {
            if (ToF_distances_matrix[i][j] == UINT16_MAX)
                DEBUG_PRINT("X ");
            else
                DEBUG_PRINT("%d ", ToF_distances_matrix[i][j]);
        }
        DEBUG_PRINT("\n");
        }
    //print the status matrix
    DEBUG_PRINT("--------------------------------\n");
    DEBUG_PRINT("ToFDeck P.I. Status Matrix:\n");
    for (int i = 0; i < ROW; ++i)
    {
        for (int j = 0; j < COL; ++j)
        {
            DEBUG_PRINT("%d ", ToF_status[j+COL*i]);
        }
        DEBUG_PRINT("\n");
    }
    //print the reflectance matrix
    DEBUG_PRINT("--------------------------------\n");
    DEBUG_PRINT("ToFDeck P.I. Reflectance Matrix:\n");
    for (int i = 0; i < ROW; ++i)
    {
        for (int j = 0; j < COL; ++j)
        {
            DEBUG_PRINT("%d ", ToF_Reflectance[j+COL*i]);
        }
        DEBUG_PRINT("\n");
    }
    #endif
